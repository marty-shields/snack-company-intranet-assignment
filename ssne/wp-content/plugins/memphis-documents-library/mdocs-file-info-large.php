<?php
function mdocs_file_info_large($the_mdoc, $index=0, $current_cat) {
	global $post;
	ob_start();
	...
	$last_modified = gmdate($date_format,$the_mdoc['modified']);
	...
	if( $show_files) {
		...
		if($mdocs_show_new_banners) {
			...
			if($mdocs_time_to_display_banners > $days) {
				if($is_new == true) echo '<div class="mdocs-new">'.__('New','mdocs').'</div>';
				else echo '<div class="mdocs-updated">'.__('Updated','mdocs').'</div>';
			}
		}
	?>
