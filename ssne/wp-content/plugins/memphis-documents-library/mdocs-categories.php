...
function mdocs_edit_cats() {
	...
	<div class="mdocs-ds-container">
		<h2><?php _e('Folder Editor', 'mdocs'); ?> <button class="btn btn-success btn-sm" id="mdocs-add-cat" onclick="add_main_category('<?php echo intval(get_option('mdocs-num-cats')); ?>')"class="mdocs-grey-btn"><?php _e('Add Main Folder','mdocs'); ?></button></h2>
		<form  id="mdocs-cats" method="post" action="admin.php?page=memphis-documents.php&mdocs-cat=cats" data-cat-index="<?php echo get_option('mdocs-num-cats'); ?>" data-check-index="1">
			...
			<input type="submit" class="button button-primary" id="mdocs-import-submit" onclick="mdocs_reset_onleave()" value="<?php _e('Save Folders','mdocs') ?>" />
		</form>
	</div>
	<?php
...
function mdocs_update_cats() {
	if(intval(ini_get("max_input_vars")) > count($_POST, COUNT_RECURSIVE)) {
		...
		mdocs_errors(__('max_input_vars','mdocs').': '.ini_get("max_input_vars").' '.__('Input variables sent','mdocs').': '.count($_POST, COUNT_RECURSIVE), 'error');
