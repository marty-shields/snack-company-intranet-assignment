...
function mdocs_init_settings() {
	...
	add_option('mdocs-hide-subfolders', true);
...
function mdocs_update_view_private_users() {
	...
	if(!is_array($mdocs_roles)) {
		$mdocs_roles = mdocs_init_view_private();
		update_option('mdocs-view-private', mdocs_init_view_private());
	}
...
function mdocs_admin_script() {
	if(isset($_GET['page']) && $_GET['page'] == 'memphis-documents.php') {
		...
		if(is_rtl() ) {
			wp_register_style( 'mdocs-rtl-style', MDOC_URL.'/rtl.css');
			wp_enqueue_style( 'mdocs-rtl-style' );
		}
		//INLINE STYLE
		wp_enqueue_script('mdocs-admin-script');
...
function mdocs_script() {
	...
	if(isset($post)) {
		...
		if(get_post_type($post) == 'mdocs-posts' || has_shortcode( $post->post_content, 'mdocs' ) || is_home() || is_plugin_active('so-widgets-bundle/so-widgets-bundle.php')) {
			//JQUERY
			if(get_option('mdocs-disable-jquery') == false) {
				wp_enqueue_script("jquery");
			}
			//BOOTSTRAP
			if(get_option('mdocs-disable-bootstrap') == false) {
...
function mdocs_document_ready_wp() {
	global $post;
	if(isset($post)) {
		if(get_post_type($post) == 'mdocs-posts' || has_shortcode( $post->post_content, 'mdocs' ) || is_plugin_active('so-widgets-bundle/so-widgets-bundle.php') ) {
	?>
	<script type="application/x-javascript">
			jQuery( document ).ready(function() {
