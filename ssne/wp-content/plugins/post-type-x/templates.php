...
function theme_integration_wizard( $atts ) {
	...
	if ( is_ic_integration_wizard_page() ) {
		...
		$box_content = '<h4>' . __( 'Advanced Mode Test', 'post-type-x' ) . '</h4>';
		...
		if ( !isset( $_GET[ 'test_advanced' ] ) ) {
			$box_content .= '<p>' . __( 'Post Type X is currently running in Simple Mode.', 'post-type-x' ) . '</p>';
			$box_content .= '<p>' . __( 'In Simple Mode the item listing, item search and category pages are disabled (please read this Sample Item Page to fully understand the difference).', 'post-type-x' ) . '</p>';
			$box_content .= '<p>' . __( 'Please use the button below to check out how the item page looks in Automatic Advanced Mode.', 'post-type-x' ) . '</p>';
			$box_content .= '<p class="wp-core-ui"><a href="' . esc_url( add_query_arg( 'test_advanced', '1' ) ) . '" class="button-primary">' . __( 'Start Advanced Mode Test', 'post-type-x' ) . '</a><a href="' . esc_url( add_query_arg( 'test_advanced', 'simple' ) ) . '" class="button-secondary">' . __( 'Use Simple Mode', 'post-type-x' ) . '</a></p>';
			...
			$box_content .= '<p>' . __( 'Advanced Mode is temporary enabled for this page now.', 'post-type-x' ) . '</p>';
			...
			$box_content .= '<p style="margin-bottom: 0">' . __( 'Make some adjustments if necessary (you can change it at any time later)', 'post-type-x' ) . ':</p>';
			$box_content .= '<table class="styling-adjustments">';
			$integration_settings = get_integration_settings();
			$box_content .= implecode_settings_number( __( 'Width', 'post-type-x' ), 'container_width', $integration_settings[ 'container_width' ], '%', 0, null, null, 0 );
			...
			$box_content .= implecode_settings_number( __( 'Padding', 'post-type-x' ), 'container_padding', $integration_settings[ 'container_padding' ], 'px', 0, null, null, 0 );
			if ( !defined( 'AL_SIDEBAR_PLUGIN_BASE_PATH' ) ) {
				$box_content .= implecode_settings_radio( __( 'Default Sidebar', 'post-type-x' ), 'default_sidebar', $integration_settings[ 'default_sidebar' ], array( 'none' => __( 'Disabled', 'post-type-x' ), 'left' => __( 'Left', 'post-type-x' ), 'right' => __( 'Right', 'post-type-x' ) ), 0 );
	...
	default_sidebar = jQuery("input[name=\"default_sidebar\"]:checked").val();
		...
		jQuery.post("' . admin_url( 'admin-ajax.php' ) . '", data, function(response) {
			...
			$box_content .= '<p class="wp-core-ui"><a href="' . esc_url( add_query_arg( 'test_advanced', 'ok' ) ) . '" class="button-primary integration-ok">' . __( 'It\'s Fine', 'post-type-x' ) . '</a><a href="' . esc_url( add_query_arg( 'test_advanced', 'bad' ) ) . '" class="button-secondary">' . __( 'It\'s Broken', 'post-type-x' ) . '</a></p>';
			...
			$box_content .= '<p>' . __( 'Congratulations! Post Type X is working on Advanced Mode now. You can go to admin and add the items to the catalog.', 'post-type-x' ) . '</p>';
			$box_content .= '<p>' . __( 'If you are a developer or would like to have full control on the item pages templates we still recommend to proceed with manual integration.', 'post-type-x' ) . '</p>';
			$box_content .= '<p>' . __( 'You can switch between modes at any time in Item Settings.', 'post-type-x' ) . '</p>';
			$box_content .= '<p class="wp-core-ui"><a href="' . admin_url( 'edit.php?post_type=al_product' ) . '" class="button-primary">' . __( 'Go to Admin', 'post-type-x' ) . '</a><a target="_blank" href="https://implecode.com/wordpress/product-catalog/theme-integration-guide/#cam=advanced-mode&key=integration-advanced-success" class="button-secondary">' . __( 'Free Theme Integration Guide', 'post-type-x' ) . '</a></p>';
			...
			$box_content .= '<p>' . __( 'You can switch between modes at any time in Item Settings.', 'post-type-x' ) . '</p>';
			$box_content .= '<p>' . __( 'Use the buttons below to try the advanced integration again or go to admin and start adding your items.', 'post-type-x' ) . '</p>';
...
function get_integration_settings() {
	...
	if ( function_exists( 'is_ic_price_enabled' ) ) {
...
function save_wizard_advanced_mode_settings() {
	if ( current_user_can( 'manage_product_settings' ) ) {
		...
		if ( isset( $_POST[ 'disable_attributes' ] ) ) {
...
function is_theme_implecode_supported() {
	$template	 = get_option( 'template' );
	$return		 = false;
	if ( in_array( $template, implecode_supported_themes() ) || current_theme_supports( 'post-type-x' ) ) {
		$return = true;
	}
	return $return;
}
...
/**
 * Redirects the item listing page to homepage catalog if necessary
 *
 * @param type $template
 * @return type
 */
