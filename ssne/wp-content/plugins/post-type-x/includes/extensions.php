...
function product_extensions() {
	...
	<div id="implecode_settings" class="wrap">
		<h2><?php echo sprintf( __( 'Extensions for %s', 'post-type-x' ), 'Post Type X' ) ?></h2>
		<h3><?php _e( 'All the extensions come with premium email support provided by dev team.<br>Feel free to contact us for configuration help, troubleshooting, installation assistance and any other plugin support at any time!', 'post-type-x' ) ?></h3>
		<h2 class="nav-tab-wrapper">
			<a id="extensions" class="nav-tab"
			   href="<?php echo admin_url( 'edit.php?post_type=al_product&page=extensions.php&tab=product-extensions' ) ?>"><?php _e( 'POPULAR', 'post-type-x' ); ?></a>
			<a id="new-extensions" class="nav-tab"
			   href="<?php echo admin_url( 'edit.php?post_type=al_product&page=extensions.php&tab=free-product-extensions' ) ?>"><?php _e( 'FREE', 'post-type-x' ); ?></a>
		...
		<div class="table-wrapper">
			...
			if ( $tab == 'product-extensions' OR $tab == '' ) {
				...
				<div class="extension-list">
					...
					if ( false === ($extensions = get_transient( 'implecode_typex_extensions_data' )) ) {
						$extensions = wp_remote_get( 'http://app.implecode.com/index.php?provide_extensions&type_x=1' );
						if ( !is_wp_error( $extensions ) || 200 != wp_remote_retrieve_response_code( $extensions ) ) {
							...
							if ( $extensions ) {
								set_transient( 'implecode_typex_extensions_data', $extensions, 60 * 60 * 24 * 7 );
							}
						} else {
			...
			} else if ( $tab == 'free-product-extensions' ) {
				?>
				<div class="extension-list">
					<script>
						jQuery( '.nav-tab-wrapper a' ).removeClass( 'nav-tab-active' );
					...
					if ( false === ($extensions = get_transient( 'implecode_free_extensions_data' )) ) {
						$extensions = wp_remote_get( 'http://app.implecode.com/index.php?provide_extensions&free=1' );
						if ( !is_wp_error( $extensions ) || 200 != wp_remote_retrieve_response_code( $extensions ) ) {
							$extensions = json_decode( wp_remote_retrieve_body( $extensions ), true );
							if ( $extensions ) {
								set_transient( 'implecode_free_extensions_data', $extensions, 60 * 60 * 24 * 7 );
							}
						} else {
							$extensions = implecode_extensions();
						}
					...
					echo extensions_bundle_box();
					foreach ( $extensions as $extension ) {
						echo free_extension_box( $extension[ 'name' ], $extension[ 'url' ], $extension[ 'desc' ], $extension[ 'comp' ], $extension[ 'slug' ], $all_ic_plugins, $not_active_ic_plugins );
					}
					?>
				</div>
				<div class="helpers">
				...
				<div class="help">
					...
					echo '<p>In case you prefer to install the extension manually you will get also the installation files by email. See <a href="https://implecode.com/wordpress/product-catalog/plugin-installation-guide/?cam=typex-extensions-help&key=manual-installation#manual">manual installation guide</a> for this.</p>';
					echo '<p>Please see the <a href="https://implecode.com/faq/?cam=typex-extensions-help&key=faq">FAQ</a> for additional information</p>';
					?>
				</div>
				<div class="helpers">
					<div class="wrapper"><h2><?php _e( 'Did you Know?', 'post-type-x' ) ?></h2><?php
		...
		<div class="plugin-logo">
			<a href="https://implecode.com/#cam=typex-catalog-settings-link&key=logo-link"><img class="en" src="<?php echo AL_PLUGIN_BASE_PATH . 'img/implecode.png'; ?>" width="282px" alt="impleCode"/></a>
		</div>
	</div>
	<?php
}
...
function extension_box( $name, $url, $desc, $comp = 'simple', $slug, $all_ic_plugins, $not_active_ic_plugins ) {
	...
	<a class="extension-name" href="https://implecode.com/wordpress/plugins/' . $url . '/#cam=typex-extensions&key=' . $url . '"><h3><span>' . $name . '</span></h3><span class="click-span">' . __( 'Click for more', 'post-type-x' ) . '</span></a>
	<p>' . $desc . '</p>';
	$disabled	 = '';
	...
	if ( !empty( $all_ic_plugins ) && is_ic_plugin_active( $slug, $all_ic_plugins ) ) {
		...
		if ( $comp_class == 'wrong' ) {
			$return .= '<p><a href="https://implecode.com/wordpress/plugins/' . $url . '/#cam=typex-extensions&key=' . $url . '" class="button-primary">See the Extension</a><span class="comp ' . $comp_class . '">' . $comp_txt . '</span></p>';
		} else {
			...
			$return .= '<p class="submit"><input type="submit" ' . $disabled . ' value="Install" class="button-primary"><span class="comp ' . $comp_class . '">' . $comp_txt . '</span> <a href="https://implecode.com/wordpress/plugins/' . $url . '/#cam=typex-extensions&key=' . $url . '" class="button-secondary right">Get your key</a></form></p>';
		}
	}
	$return .= '</div>';
	return $return;
	...
	if ( isset( $_GET[ 'implecode_install' ] ) && !empty( $_GET[ 'slug' ] ) && !empty( $_GET[ 'license_key' ] ) && wp_verify_nonce( $_GET[ '_wpnonce' ], 'install-implecode-plugin_' . $_GET[ 'slug' ] ) == 1 && current_user_can( 'install_plugins' ) ) {
		...
		if ( $api != 'error' ) {
			...
			echo '<div id="message error" class="error product-adder-message messages-connect">
				<div class="squeezer">
					<h4><strong>' . sprintf( __( 'This is not a valid license key! Get it <a href="%s">here</a>.', 'post-type-x' ), 'https://implecode.com/wordpress/plugins/' . $_GET[ 'url' ] . '/#cam=typex-extensions&key=' . $_GET[ 'url' ] ) . '</strong></h4>
				</div>
			</div>';
		} else {
			echo '<div id="message error" class="error product-adder-message messages-connect">
				<div class="squeezer">
					<h4><strong>' . sprintf( __( 'The supplied license key is not valid for this extension! Upgrade it <a href="%s">here</a>.', 'post-type-x' ), 'https://implecode.com/wordpress/plugins/' . $_GET[ 'url' ] . '/#cam=typex-extensions&key=' . $_GET[ 'url' ] ) . '</strong></h4>
				</div>
			</div>';
		}
		...
		echo '<div id="message error" class="error product-adder-message messages-connect">
				<div class="squeezer">
					<h4><strong>' . sprintf( __( 'You need to provide the license key to activate the extension. Get yours <a href="%s">here</a>.', 'post-type-x' ), 'https://implecode.com/wordpress/plugins/' . $_GET[ 'url' ] . '/#cam=typex-extensions&key=' . $_GET[ 'url' ] ) . '</strong></h4>
				</div>
			</div>';
...
function implecode_install_actions( $install_actions, $api, $plugin_file ) {
	$install_actions[ 'plugins_page' ] = '<a href="' . admin_url( 'edit.php?post_type=al_product&page=extensions.php&tab=product-extensions' ) . '">' . __( 'Reload the Page', 'post-type-x' ) . '</a>';
...
function extensions_bundle_box() {
	if ( !function_exists( 'start_implecode_updater' ) ) {
		echo '<div class="bundle-box">' . __( 'Do you need multiple extensions?', 'post-type-x' ) . ' <a href="https://implecode.com/choose-a-plan/#cam=typex-bundles&key=typex-extensions-bundle-box">' . __( 'Check out extensions bundles', 'post-type-x' ) . '</a></div>';
	}
}
