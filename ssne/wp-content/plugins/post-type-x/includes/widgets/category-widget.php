...
/**
 * Manages item category widget
 *
 * Here item category widget is defined.
 *
 * @version        1.4.0
...
class product_cat_widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'widget_product_categories widget_categories', 'description' => __( 'A list or dropdown of item categories', 'post-type-x' ) );
		parent::__construct( 'product_categories', __( 'Item Categories', 'post-type-x' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		if ( get_integration_type() != 'simple' ) {
			...
			if ( $d ) {
				$cat_args = array( 'orderby' => 'name', 'show_count' => $c, 'hierarchical' => $h, 'taxonomy' => 'al_product-cat', 'walker' => new my_Walker_CategoryDropdown, 'show_option_none' => __( 'Select Item Category', 'post-type-x' ) );
	...
	function form( $instance ) {
		...
		if ( get_integration_type() != 'simple' ) {
			...
			<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'dropdown' ); ?>"
				...
				<label
					for="<?php echo $this->get_field_id( 'count' ); ?>"><?php _e( 'Show item counts', 'post-type-x' ); ?></label><br/>
			...
			if ( is_integration_mode_selected() ) {
				implecode_warning( sprintf( __( 'Category widget is disabled with simple theme integration. Please see <a href="%s">Theme Integration Guide</a> to enable item category widget.', 'post-type-x' ), 'https://implecode.com/wordpress/product-catalog/theme-integration-guide/#cam=simple-mode&key=category-widget' ) );
			} else {
				implecode_warning( sprintf( __( 'Category widget is disabled due to a lack of theme integration.%s', 'post-type-x' ), sample_product_button( 'p' ) ) );
			}
		}
	}
