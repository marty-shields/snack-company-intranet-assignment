...
/**
 * Manages item settings
 *
 * Here item settings are defined and managed.
 *
 * @version		1.1.4
...
function product_settings() {
	...
	<div id="implecode_settings" class="wrap">
		<h2><?php _e( 'Settings', 'post-type-x' ) ?> - impleCode Post Type X</h2>
		...
		do_action( 'settings-content', $tab );
		?>
...
function doc_helper( $title, $url, $class = null ) {
	...
	__( '%s Settings in Docs', 'post-type-x' ), ucfirst( $title ) ) . '</div>
		<div class="doc-description">' . sprintf(
	__( 'See %s configuration tips in the impleCode documentation', 'post-type-x' ), $title ) . '.</div>
		<div class="doc-button"><a href="https://implecode.com/docs/post-type-x/' . $url . '/?cam=catalog-docs-box&key=' . $url . '"><input class="doc_button classic-button" type="button" value="' . __( 'See in Docs', 'post-type-x' ) . '"></a></div>
		<a title="' . __( 'Click the button to visit impleCode documentation', 'post-type-x' ) . '" href="https://implecode.com/docs/post-type-x/' . $url . '/?cam=catalog-docs-box&key=' . $url . '" class="background-url"></a>
		</div></div>';
	echo $helper;
}
...
function review_helper() {
	$helper = '<div class="doc-helper review"><div class="doc-item">
		...
		<div class="doc-description">' . sprintf( __( 'Please <a href="%s">rate</a> this plugin and tell me if it works for you or not. It really helps development.', 'post-type-x' ), 'https://wordpress.org/support/view/plugin-reviews/post-type-x#postform' ) . '</div>
		</div></div>';
	echo $helper;
}
...
/**
 * Returns all Post Type X option names
 * (needs optimisation)
 * @return type
 */
function all_ic_options( $which = 'all' ) {
	...
	$tools	 = array( 'ic_delete_products_uninstall', 'ecommerce_product_catalog_ver', 'sample_product_id', 'al_permalink_options_update', 'custom_license_code', 'implecode_license_owner', 'no_implecode_license_error', 'license_active_plugins', 'product_adder_theme_support_check', 'implecode_hide_plugin_review_info_count', 'hide_empty_bar_message', 'PTX_first_activation_version', 'first_activation_version', 'post_type_x_ver' );
	if ( $which == 'all' ) {
