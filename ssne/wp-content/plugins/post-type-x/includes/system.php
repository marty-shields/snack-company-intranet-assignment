	...
	if ( current_user_can( 'manage_product_settings' ) ) {
		if ( isset( $_GET[ 'reset_product_settings' ] ) ) {
			if ( isset( $_GET[ 'reset_product_settings_confirm' ] ) ) {
				...
				implecode_success( __( 'Item Settings successfully reset to default!', 'post-type-x' ) );
			} else {
				echo '<h3>' . __( 'All items settings will be reset to defaults. Would you like to proceed?', 'post-type-x' ) . '</h3>';
			...
			if ( isset( $_GET[ 'delete_all_product_categories_confirm' ] ) ) {
				...
				implecode_success( __( 'All Item Categories successfully deleted!', 'post-type-x' ) );
			} else {
				echo '<h3>' . __( 'All item categories will be permanently deleted. Would you like to proceed?', 'post-type-x' ) . '</h3>';
			...
			<table class="widefat" cellspacing="0" id="ic_tools">
				...
				<tbody>
					<tr>
						<td><?php _e( 'Reset item settings', 'post-type-x' ); ?>:</td>
						<td><a class="button" href="<?php echo esc_url( add_query_arg( 'reset_product_settings', 1 ) ) ?>"><?php _e( 'Reset Item Settings', 'post-type-x' ) ?></a></td>
					</tr>
					<tr>
						<td><?php _e( 'Delete all items', 'post-type-x' ); ?>:</td>
						<td><a class="button" href="<?php echo esc_url( add_query_arg( 'delete_all_products', 1 ) ) ?>"><?php _e( 'Delete all Items', 'post-type-x' ) ?></a></td>
					</tr>
					<tr>
						<td><?php _e( 'Delete all item categories', 'post-type-x' ); ?>:</td>
						<td><a class="button" href="<?php echo esc_url( add_query_arg( 'delete_all_product_categories', 1 ) ) ?>"><?php _e( 'Delete all Item Categories', 'post-type-x' ) ?></a></td>
					</tr>
					<tr>
			...
			<table class="widefat" cellspacing="0" id="status">
				...
				<tbody>
					...
					<tr>
						<td><?php
							_e( 'Post Type X Version', 'post-type-x' );
							?>:
						</td>
						<td>
							<?php
					...
					<tr>
						<td><?php _e( 'Post Type X Database Version', 'post-type-x' ); ?>:</td>
						<td><?php echo get_option( 'post_type_x_ver', $plugin_version ); ?></td>
					</tr>
					<tr>
			...
			<table class="widefat" cellspacing="0">
				...
				<tbody>
					...
					<tr>
						...
						<td><?php
							echo is_child_theme() ? '<mark class="yes">' . '&#10004;' . '</mark>' : '&#10005; &ndash; ' . sprintf( __( 'If you\'re modifying Post Type X or a parent theme you didn\'t build personally we recommend using a child theme. See: <a href="%s" target="_blank">How to create a child theme</a>', 'post-type-x' ), 'http://codex.wordpress.org/Child_Themes' );
							?></td>
					</tr>
					<?php
					if ( is_child_theme() ) :
					...
					<?php endif ?>
					<tr>
						<td><?php _e( 'Post Type X Support', 'post-type-x' ); ?>:</td>
						<td><?php
