...
/**
 * The template for displaying items archive content.
 *
 * @version		1.1.3
...
<article id="product_listing" <?php post_class( $listing_class ); ?>>
	...
	<div class="entry-content">
		...
		} else if ( (((function_exists( 'is_product_filters_active' ) && !is_product_filters_active()) || !function_exists( 'is_product_filters_active' )) && is_search()) && !more_products() ) {
			echo '<p>' . __( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'post-type-x' ) . '</p>';
			product_search_form();
		} else if ( function_exists( 'is_product_filters_active' ) && is_product_filters_active() && !more_products() ) {
			show_product_sort_bar();
			$filters = get_active_product_filters();
			if ( isset( $_GET[ 's' ] ) ) {
				$filters[] = 's';
			}
			echo '<p>' . sprintf( __( 'Sorry, but nothing matched your search terms. Please try again with some different options or %sreset filters%s.', 'post-type-x' ), '<a href="' . esc_url( remove_query_arg( $filters ) ) . '">', '</a>' ) . '</p>';
		}
		?>
	</div>
</article><?php
