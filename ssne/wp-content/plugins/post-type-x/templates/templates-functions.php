...
/**
 * Sets item limit on item listing pages
 * @param object $query
 */
...
/**
 * Adds paginaion to the item listings
 *
...
/**
 * Ads item listing inline styles container
 */
...
/**
 * Ads item page inline styles container
 */
...
/**
 * Returns item listing template defined in settings
 *
 * @return string
 */
...
/**
 * Replaces auto items listing, item category pages and item search title with appropriate entries
 *
...
function override_product_page_title( $page_title, $id = null ) {
	if ( !is_admin() && is_ic_catalog_page() && !is_ic_product_page() && !in_the_loop() && !is_ic_shortcode_query() && (empty( $id ) || (get_quasi_post_type( get_post_type( $id ) ) == 'al_product')) ) {
		...
		if ( is_ic_taxonomy_page() ) {
			...
			$page_title = __( 'Search Results for:', 'post-type-x' ) . ' ' . $_GET[ 's' ];
...
/**
 * Adds item post type navigation menu current class
 *
 * @global type $post
...
/**
 * Adds items post type navigation class for automatic main menu
 *
 * @global type $post
 * @param string $classes
 * @param type $page
...
/**
 * Defines custom classes to item or category listing div
 * @return string
 */
...
/**
 * Defines custom classes to item or category element div
 * @return string
 */
...
/**
 * Adds item main categories label on item listing
 *
 */
...
/**
 * Adds item subcategories label on category item listing
 *
 */
...
/**
 * Adds item header on item listing
 *
 */
